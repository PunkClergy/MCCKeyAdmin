<script>
	export default {
		...
		// 全局数据（替代原 data，UniApp 推荐用 globalData）
		globalData: {
		...
		// UniApp 生命周期：应用初始化（替代原 onLaunch）
		onLaunch(options) {
			// 1. 检查更新（仅微信小程序端生效，其他端自动忽略）
			...
			// 2. 读取本地存储（UniApp 统一用 uni.getStorageSync，多端兼容）
			const userInfo = uni.getStorageSync(this.globalData.userKey);
			if (userInfo) {
				this.globalData.userInfo = userInfo;
			}
			const k1swUrl = uni.getStorageSync(this.globalData.k1swUrlKey);
			if (k1swUrl) {
				this.globalData.k1swUrl = k1swUrl;
			}
			const fin3Url = uni.getStorageSync(this.globalData.fin3UrlKey);
			if (fin3Url) {
				this.globalData.fin3Url = fin3Url;
			}
		},

		// UniApp 生命周期：应用显示（从前台进入/启动）
		onShow(options) {
			// 小程序启动，或从后台进入前台显示时执行
			console.log('小程序显示', options);
		},

		// UniApp 生命周期：应用隐藏（进入后台）
		onHide() {
			// 小程序从前台进入后台时执行
			console.log('小程序隐藏');
		},
