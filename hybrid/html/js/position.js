...
/**
 * 地址解析：经纬度转对应语言地址（谷歌Geocoding API）
 * @param {number} lat
 * @param {number} lng
...
async function resolveAddress(lat, lng) {
	try {
		// zhCn → zh-cn，enUs → en-us，适配google语言参数格式
		const mapLang = currentLang.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
		const url = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${GOOGLE_API_KEY}&language=${mapLang}`;
