	...
	<view class="container">
		<view class="record-container">
			...
			<view class="record-tabs">
				<view class="record-tabs-item" :class="c_activeTab === 1 ? 'tab-active' : ''" @tap="handleSwitchTab(1)">
					{{tips.RoleList[lang]}}
				</view>
				<view class="record-tabs-item" :class="c_activeTab === 2 ? 'tab-active' : ''" @tap="handleSwitchTab(2)">
					{{ btnState }}{{tips.Role[lang]}}
				</view>
			</view>

			<!-- 列表 -->
			<block v-if="c_activeTab === 1">
				<scroll-view scroll-y class="list-scroll" @scrolltolower="handleLower">
					<view class="content-item" v-for="(item, index) in g_items" :key="index">
						<view class="content-item-head">
							...
							<view class="head-right">
								<image src="/static/images/_edit.png" @tap="handleEdit(item)" />
								<image src="/static/images/_delete.png" @tap="handleDelete(item.id)" />
							</view>
						</view>
				...
				<scroll-view scroll-y class="form-scroll">
					<view class="long-range-control-car">
						<view class="card-head">
							<text>{{tips.BasicInfo[lang]}}</text>
						</view>

						<view class="long-range-control-car-subitem">
							<label>{{tips.RoleName[lang]}}<text>*</text></label>
							<view class="card-info-item-input">
								<input :placeholder="tips.EnterRoleName[lang]" v-model="params.name" />
							</view>
						</view>
						...
						<view class="card-head">
							<text>{{tips.PermissionSettings[lang]}}</text>
						</view>
							...
							<view class="tree-container">
								<view class="node" v-for="item in tree" :key="item.id">
									...
									<view class="node-header">
										<view class="arrow" :class="item.isExpanded ? 'expanded' : ''"
											v-if="item.children.length" @tap="toggleExpand(item.id)">
											▶
										</view>
										<view class="checkbox"
									...
									<view class="children" v-if="item.children.length && item.isExpanded">
										<view class="child-node" v-for="child1 in item.children" :key="child1.id">
											<view class="node-header">
												<view class="arrow" :class="child1.isExpanded ? 'expanded' : ''"
													v-if="child1.children.length" @tap="toggleExpand(child1.id)">
													▶
												</view>
												<view class="checkbox"
											...
											<view class="children" v-if="child1.children.length && child1.isExpanded">
												<view class="child-node" v-for="child2 in child1.children"
													:key="child2.id">
													<view class="node-header">
														<view class="arrow" :class="child2.isExpanded ? 'expanded' : ''"
													...
													<!-- 四级 -->
													<view class="children"
														v-if="child2.children.length && child2.isExpanded">
														<view class="child-node" v-for="child3 in child2.children"
															:key="child3.id">
															<view class="node-header">
																<view class="arrow"
																...
																<view class="checkbox"
																	...
																	<view class="checkmark" v-if="child3.checked">✓
																	</view>
																	<view class="indeterminate-line"
																		v-if="child3.indeterminate"></view>
																</view>
															...
															<!-- 五级 -->
															<view class="children"
																v-if="child3.children.length && child3.isExpanded">
																<view class="child-node"
																	v-for="child4 in child3.children" :key="child4.id">
																	<view class="node-header">
																		<view class="checkbox"
																			:class="{ checked: child4.checked }"
																			@tap="handleCheck(child4.id)">
																			<view class="checkmark"
																				v-if="child4.checked">✓</view>
																		</view>
...
<script>
	...
	export default {
		...
		onShow() {
			...
			const pageRoute = 'zoneCenter/rolePermission'
